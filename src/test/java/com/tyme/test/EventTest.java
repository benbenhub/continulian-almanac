...
public class EventTest {
  ...
  public void test0() {
    ...
    Assert.assertEquals("2008年2月29日", d.toString());

    // 2005年没有2月29，按最初设置的，没有就倒推1天
    d = e.getSolarDay(2005);
    Assert.assertEquals("2005年2月28日", d.toString());

    e = Event.fromName("农历生日");
    Assert.assertNotNull(e);

    d = e.getSolarDay(2026);
    Assert.assertEquals("2026年6月6日", d.toString());
  }
  ...
  public void test2() {
    ...
    Assert.assertNotNull(e);
    Assert.assertEquals("国际消费者权益日", e.getName());

    SolarDay d = e.getSolarDay(2026);
    Assert.assertEquals("2026年3月15日", d.toString());

    List<Event> events = Event.fromSolarDay(d);
    Assert.assertEquals("[国际消费者权益日]", events.toString());

    // 打印2026年的所有事件
    for (Event event : Event.all()) {
      System.out.println(event + " = " + event.getSolarDay(2026));
    }
  }

  @Test
  public void test3() {
    Event e = Event.fromName("全国中小学生安全教育日");
    Assert.assertNotNull(e);
    Assert.assertEquals("全国中小学生安全教育日", e.getName());
    ...
    for (Map.Entry<Integer, String> entry : days.entrySet()) {
      SolarDay d = e.getSolarDay(entry.getKey());
      Assert.assertNotNull(d);
      Assert.assertEquals(entry.getValue(), d.toString());
    }
  }
}
